...
| `pj_noop` (crossing, no work) | 2.3 ns | 100% |
| `pj_price_european` (~200ns of work) | 56.5 ns | 4% |
| `pj_price_american(512)` (~1ms of work) | 2.74 ms | 0.00% |
...
**P1 -- CONTRADICTED.** The crossing itself costs 2.3 ns, which is indeed small. But it is not a rounding error, it is a **ratio**. Against `pj_price_european` it is 4% of the call; against `pj_price_american(512)` it is 0.00%. The same boundary is 48592x more expensive for one function than the other, and neither number is a property of the boundary. The design rule that follows is not "P/Invoke is cheap" but **put enough work behind each crossing that the crossing stops mattering** -- which is a statement about API shape, not about performance tuning.
...
|---|---:|---:|
| one call per option | 4000 | 262.50 us |
| one call for the book | 1 | 253.40 us |
...
| `pj_burn(60ms)`, SuppressGCTransition | 54.0 ms |
...
|---|---|---|---:|---:|
| batch, pinned in place | batch | no | 245.60 us | 1.00x |
| per-call, blittable pointer | per-call | no | 260.10 us | 1.06x |
| per-call, SuppressGCTransition | per-call | no | 261.30 us | 1.06x |
| batch, hand-copied to unmanaged memory | batch | no | 343.70 us | 1.40x |
| batch, marshalled array | batch | yes | 366.30 us | 1.49x |
| per-call, marshalled class | per-call | yes | 470.40 us | 1.92x |
| per-call, raw function pointer | per-call | no | 527.50 us | 2.1x |
...
**P5 -- CONTRADICTED.** The marshaller is not the axis that matters. A **marshalled** array and a hand-pinned span differ by 1.49x -- because the element type is blittable, the marshaller pins the array rather than copying it, and does almost exactly what the hand-written code does. Meanwhile a marshalled **class** costs 1.81x a blittable per-call pointer with the identical crossing count, because a reference type can never be passed in place and must be allocated, copied and freed every time. The rule is not "avoid the marshaller". It is **avoid non-blittable types, and avoid crossing more often than you must** -- and of the two, the crossing count dominates.

## P12 -- the ABI's shape is the performance decision

| method | crossings | median | delta error vs analytic |
|---|---:|---:|---:|
| `pj_greeks_european` | 1 | 128.9 ns | exact |
| bump and reprice | 9 | 551.8 ns | 1.9E-011 |
