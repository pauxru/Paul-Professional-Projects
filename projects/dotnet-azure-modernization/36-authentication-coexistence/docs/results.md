...
| --- | ---: | ---: |
| MembershipSha1 | 0.024 | 362.324 |
| IdentityV2 | 0.626 | 373.928 |
| IdentityV3 | 1.524 | 365.293 |
| Argon2id | 368.509 | 371.346 |

Argon2id costs **588x** a PBKDF2-HMAC-SHA1 verification at 1000 iterations, and that ratio counts only time. The memory term -- 64 MiB per verification against a few hundred bytes -- is the part an attacker with a warehouse of GPUs actually feels, and it does not appear in this column at all.

| measurement | no padding | constant work | base rate |
| --- | ---: | ---: | ---: |
| four-way format identification | 100.0% | 30.8% | 25.0% |
...
| ---: | ---: | ---: | ---: | ---: |
| 1 | 17.9% | 66.4 ms | 368.5 ms | 5.55x |
| 30 | 72.5% | 267.2 ms | 368.5 ms | 1.38x |
| 90 | 85.5% | 315.1 ms | 368.5 ms | 1.17x |
| 365 | 93.7% | 345.3 ms | 368.5 ms | 1.07x |
| 1095 | 95.7% | 352.8 ms | 368.5 ms | 1.04x |
...
| 5 | Argon2id at the deployable RFC 9106 profile will cost between 30x and 100x a PBKDF2-HMAC-SHA1 verification at 1000 iterations. | contradicted | 588x |
...
| 7 | With constant-work padding enabled, that accuracy falls to roughly chance (25% for a four-way choice). | held | 30.8% four-way (chance 25.0%), 69.2% binary (base rate 75.0%) |
